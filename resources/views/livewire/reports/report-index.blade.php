<div>
    ...
    @if ($tab === 'sales')
        ...
        <div class="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-5">
            ...
            <div class="xl:col-span-2 flex flex-col justify-between rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                <div>
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900">Produk Terlaris</h3>
                            <p class="text-xs text-slate-500">Peringkat produk berdasarkan kuantiti terjual</p>
                        </div>
                        @if ($topProducts->total() > 0)
                            <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
                                {{ $topProducts->total() }} Produk
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="mt-5 space-y-3">
                        @forelse ($topProducts as $product)
                            @php
                                $rank = ($topProducts->currentPage() - 1) * $topProducts->perPage() + $loop->iteration;
                            @endphp
                            <div class="rounded-2xl border border-slate-100 p-4 transition hover:border-slate-200 hover:bg-slate-50/50">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="flex items-start gap-3">
                                        <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg {{ $rank <= 3 ? 'bg-amber-100 text-amber-800 font-black' : 'bg-slate-100 text-slate-600 font-bold' }} text-xs">
                                            #{{ $rank }}
                                        </span>
                                        <div>
                                    ...
                                    <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700 shrink-0">
                                        {{ number_format($product->total_quantity, 0, ',', '.') }} terjual
                                    </span>
                                </div>
                                <div class="mt-3 flex items-center justify-between border-t border-slate-50 pt-2 text-xs text-slate-500">
                                    <span>Total Penjualan</span>
                                    <span class="text-sm font-bold text-slate-900">Rp {{ number_format($product->total_sales, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        @empty
                ...
                @if ($topProducts->hasPages())
                    <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
                        <button
                            type="button"
                            wire:click="previousPage('topProductsPage')"
                            @if ($topProducts->onFirstPage()) disabled @endif
                            class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition"
                        >
                            Sebelumnya
                        </button>
                        
                        <span class="text-xs font-bold text-slate-500">
                            Hal {{ $topProducts->currentPage() }} dari {{ $topProducts->lastPage() }}
                        </span>

                        <button
                            type="button"
                            wire:click="nextPage('topProductsPage')"
                            @if (!$topProducts->hasMorePages()) disabled @endif
                            class="rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition"
                        >
                            Selanjutnya
                        </button>
